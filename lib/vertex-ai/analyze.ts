...
export async function analyzeVideo(
  ...
  try {
    ...
    // *** START: Recalculate overallScore (1-10 scale) based on AI categories ***
    console.log('Recalculating overall score (1-10 scale) based on AI categories...');
    // Store the original 1-100 score for fallback calculation
    const originalOverallScore100 = analysis.overallScore; 

    // Ensure scoreError is initially undefined
    analysis.scoreError = undefined;

    if (analysis.categories && analysis.categories.length > 0) {
      const sumOfCategoryScores = analysis.categories.reduce((sum, category) => {
        // Ensure score is a number (1-10) and default to 0 if missing or invalid
        const score = (typeof category.score === 'number' && category.score >= 1 && category.score <= 10) ? category.score : 0;
        // Add score to sum only if it was valid (non-zero)
        return score > 0 ? sum + score : sum;
      }, 0);
      // Count only categories that had a valid score
      const validCategoryCount = analysis.categories.filter(c => typeof c.score === 'number' && c.score >= 1 && c.score <= 10).length;

      if (validCategoryCount > 0) {
          const averageCategoryScore = sumOfCategoryScores / validCategoryCount;
          // Use the rounded average score (1-10) as the new overall score
          analysis.overallScore = Math.round(averageCategoryScore);
          console.log('Updated overall score based on AI categories (1-10 scale):', {
            newScore: analysis.overallScore,
            numberOfValidCategories: validCategoryCount,
            averageCategoryScore: averageCategoryScore.toFixed(2) 
          });
      } else {
           // If no valid category scores, set score to null and add error message
           analysis.overallScore = null;
           analysis.scoreError = 'Score calculation failed: No valid category scores returned by AI.';
           console.warn(analysis.scoreError);
      }

    } else {
      // If no AI categories array, set score to null and add error message
      analysis.overallScore = null;
      analysis.scoreError = 'Score calculation failed: No categories were generated by AI.';
      console.warn(analysis.scoreError);
    }
    // *** END: Recalculate overallScore (1-10 scale) ***
